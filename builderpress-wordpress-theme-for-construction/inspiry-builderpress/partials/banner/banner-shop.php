...
$in_page_banner_title          = get_post_meta( wc_get_page_id( 'shop' ), 'bp-banner-in-page-title', true );
$in_page_banner_title_color    = get_post_meta( wc_get_page_id( 'shop' ), 'bp-banner-in-page-title-color', true );
$in_page_banner_bg_image       = get_post_meta( wc_get_page_id( 'shop' ), 'bp-banner-in-page-bg-image', true );
$in_page_banner_display_option = get_post_meta( wc_get_page_id( 'shop' ), 'bp-banner-display-option', true );
...
if ( $in_page_banner_display_option !== 'no' ) {
	...
	if ( ! empty( $in_page_banner_title ) || ! empty( $in_page_banner_title_color ) ) {
		...
		if ( ! empty( $in_page_banner_title ) ) {
			...
			$in_page_shop_banner_title = woocommerce_page_title();
		}
